...
function! s:vimim_debug(...)
    ...
    sil!echo "\n::::::::::::::::::::::::"
    if len(a:000) > 1
        sil!echo join(map(deepcopy(a:000, 1), 'string(v:val)'), "\n")
...
function! s:vimim_dictionary_keycodes()
    ...
    let s:keycodes.wubi     = "[a-z]"       " @TODO: z?
    ...
    let ime .= ' wubi98 wubi2000 wubijd wubiqq wubihf'
...
function! s:vimim_set_title(title)
    ...
    " Why do we check this? Option 'statusline' doesn't need 'term'.
    " @TODO: FIXME
    " @BUG: after enter <c-_><c-^>, vimim can't restore the statusline.
    if &term == 'screen'
...
function! s:vimim_powerline_hook()
    let l:colorschemes = Pl#Colorscheme#Init([
        \ Pl#Hi#Segments(['vimim:statusline'], {
            \ 'n': ['gray8', 'gray2'],
...
function! s:vimim_im_chinese()
    ...
    if s:ui.im =~ 'wubi'
        for wubi in split('wubi98 wubi2000 wubijd wubiqq wubihf')
...
function! g:vimim_single_quote()
    let key = "'"
    if s:toggle_punctuation > 0 && s:ui.im =~ 'wubi'
        let pairs = split(s:key_evils[key], '\zs')
        let s:smart_quotes.single += 1
        let yes = pumvisible() ? '\<C-Y>' : ""
        let key = yes . get(pairs, s:smart_quotes.single % 2)
        sil!exe 'sil!return "' . key . '"'
    endif
...
function! s:vimim_get_from_datafile(keyboard)
    ...
    if s:ui.im =~ 'pinyin'
        ...
        for i in range(10)
            let oneline = get(backend.lines, cursor + i)
            let code = split(oneline)[0]
            if code !~ pattern " filter
                break
            endif
...
function! s:vimim_plug_and_play()
    ...
    aug VimIM
        ...
        au VimEnter *
                    ...
                    \ | call Pl#Theme#InsertSegment('vimim:statusline',
                        \'before', 'fileformat')
                    \ | endif
        au SourcePre *autoload/Powerline/Themes/*.vim
                    \ if exists('g:Powerline_loaded') && g:Powerline_loaded
                    \ | call s:vimim_powerline_hook()
                    \ | endif
    aug END
endfunction
